...
gate, five live submissions, and a leaderboard that couldn't see any of it.*
...
**0.8406 ± 0.0073**. Public leaderboard: somewhere in a 0.746–0.766 noise
band, best displayed score held by the day-one baseline.
...
- **Repeated cross-validation** (5-fold × 10 repeats = 50 held-out
...
- **Paired t-tests on per-fold scores** for every accept/reject decision
  (improvement > 0.1% and p < 0.05, with a cheaper 25-fold screen and
  50-fold retest for borderline cases).
- **Fresh-fold re-validation** for anything found by search, so tuning
  couldn't overfit the evaluation protocol itself.
- **Nested CV** (search inside each outer fold) for the final honest number.
- **Leaderboard submissions spent like money** — 5 of today's 10, never for
  model selection.

The full pipeline is in the repo (`run_all.py`, `exputil.py`,
`results.jsonl`); this document is the story and the lessons.
...
| Tasks 1–2 | Classic features, group imputation | 0.8153 | 0.74641 |
| Task 3 | Gradient boosting, full features | 0.8203 | 0.75358 |
| Task 4 | Soft-voting ensemble | 0.8314 | 0.76076 |
| Task 5 | **Stacking (LR meta-learner)** | **0.8365** | — |
| Task 7 | Optuna-tuned ensemble members | 0.8370 | 0.75837 |
| Task 14 | Nested-CV estimate of final pipeline | 0.8406 ± 0.0073 | — |
| Tasks 6, 8–13 | Six more serious attempts | all rejected | — |

The CV line climbs +2.7% and the leaderboard never notices. That's the
experiment.
...
Random forest on `Pclass, Age, SibSp, Parch, Fare, Sex, Embarked`, median/mode
imputation inside a leak-free sklearn `Pipeline`. Nothing clever; everything
reproducible. This number became the bar every later step had to clear
*statistically*, not narratively.

### Tasks 1–2: the first mirage

I added the classic engineered features — `Title` from `Name`,
`FamilySize`/`IsAlone` from `SibSp`/`Parch`, `Deck` from `Cabin` — plus
group-based Age imputation (median per Title × Pclass).

A single 5-fold CV run said `+FamilySize` improved the model: 0.8104 vs
0.8059. Looked like a win. Repeated CV (50 fits) said the opposite: the
baseline stayed on top at 0.8134 and every feature variant sat within noise.
Group imputation (Title × Pclass, ~12 groups) was the only accepted change,
worth about +0.2%.

**Lesson 1 — single CV runs lie.** On 891 rows, the standard error of a
5-fold estimate is roughly ±1.5%. Confirm every "improvement" with repeated,
paired evaluation before believing it. This lesson shaped the rest of the
project.

### Task 3: the right model unlocks the features

HistGradientBoosting on the full feature set: 0.8203. The features from
task 1 that the random forest couldn't use — boosting exploited through
interactions the bagged trees averaged away.

**Lesson 2 — a feature's value is model-dependent.** "Feature engineering
didn't work" often means "the model couldn't express what the feature
encodes."

### Task 4: ensembles buy diversity, not accuracy

Soft voting over RF + HGB + LR: 0.8314 (+1.1% over HGB alone). The logistic
regression — clearly the *weakest* member solo — contributed the most,
because its errors look different from the trees'. Up-weighting the
strongest member made things worse; uniform weights won.

### Task 5: stacking beats voting (0.8365, p = 0.0005)

A `StackingClassifier` with a logistic meta-learner trained on out-of-fold
predictions — plus `passthrough`, so the meta-learner sees the raw features
alongside base predictions. First unambiguous win of the second phase.
...
- HGB: **0.8320 vs 0.8176 untuned (+1.44%, p < 0.0001)**
- RF: **0.8305 vs 0.8085 untuned (+2.20%, p < 0.0001)**

Tuned random forest *alone* nearly matched the previous whole ensemble. In
the stack: 0.8370 vs 0.8365, p = 0.74. No significant change.

**Lesson 4 — single-model improvements don't transfer to ensembles
...
### Tasks 9–13: the folklore features don't clear the bar
...
skipped automatically — their dependency (task 9) had been rejected, and
the runner knew it.
...
task dependencies in your runner, or you'll KeyError at 3 a.m.)

### Task 14: the honest number

Final pipeline, re-evaluated with the HPO running *inside* every outer fold
(10 outer folds × a 15-trial inner search): **0.8406 ± 0.0073**.

Slightly *above* the flat 50-fold estimate — the search never overfit the
evaluation protocol. This is the most trustworthy estimate of the pipeline's
true skill, and it says: essentially at Titanic's legitimate ceiling (~0.84;
the 73 perfect scores atop the leaderboard are overfit or junk entries).
...
### Task 16: the leaderboard can't see any of this
...
hundred scored rows, is about eight passengers of noise. The displayed rank
(~7,390 of 10,200 teams) is still held by the *baseline*, i.e., by luck of
the draw.
...
competition: build → repeated CV → believe only consistent, paired gains →
spend LB submissions sparingly as coarse sanity checks.

## The transferable lessons

1. **Single CV runs lie.** Repeated CV + paired tests, or it didn't happen.
2. **A feature's value is model-dependent**; a model's value is
   ensemble-dependent. Measure at the level you ship.
...
5. **Re-validate tuned configs on fresh folds**; report nested CV for the
   final number — it's the only estimate HPO can't inflate.
...
workers don't mix reliably). The final runner is **serial** with
...
```bash
pip install kaggle pandas scikit-learn xgboost lightgbm catboost optuna
kaggle competitions download -c titanic -p data && cd data && unzip titanic.zip && cd ..
python eda.py                     # data exploration
OMP_NUM_THREADS=8 python train.py   # final champion -> submission.csv
OMP_NUM_THREADS=8 python -u run_all.py 2>&1 | tee runner.log  # full gated pipeline
```

Every gate decision is in `results.jsonl`; per-fold score arrays are saved
as `.npy` files; the champion configuration is `champion.json`.

## What I'd carry to a real competition
...
data — the discipline doesn't.

---

*Repo: [github.com/cdnwetzel/titanic](https://github.com/cdnwetzel/titanic) · Data: Kaggle Titanic: Machine Learning from
Disaster (download via `kaggle competitions download -c titanic`; not redistributed here per competition rules) · All claims in this document are backed by paired statistical
tests; the rejections are as real as the accepts.*
