"""Finish runner: tasks 10(post-hoc)-14 after the task-10 dependency crash.

Recovered state: tasks 6-9 done (all rejects; ticket features not in champion).
Task 10 (fare per person) requires TicketGroupSize -> skipped by dependency.
Task 12 (target encoding) requires ticket features -> skipped by dependency.

Runs: task 6 post-hoc (xgb+cat jointly), task 11 gate, task 13 gate,
task 14 nested CV, final artifacts.
"""
...
# Champion baseline: tuned-member stack (task 7 showed tuned == untuned, p=0.74;
# tuned kept for its fresh-CV-validated single-model gains).
log("FINISH RUN: establishing champion baseline (tuned-member stack)",
    event="task_start", task=0)
X0, y0 = load_xy()
CHAMP["scores"] = full_eval(build_stack(), X0, y0, "champion_baseline", 0,
                            "Champion stack RF+HGB+LR tuned (50-fold)")

# ================================================================ TASK 6 post-hoc
log("TASK 6 post-hoc: +xgb AND +cat jointly (both singles were positive-direction)",
    event="task_start", task=6)
gate(6, "+xgb+cat joint", spec_override={"members": ["rf", "hgb", "lr", "xgb", "cat"]},
     event="posthoc_joint")
log("TASK 6 post-hoc done.", event="task_done", task=6)

# ================================================================ dependency skips
log("TASK 10 skipped: requires TicketGroupSize (task 9 rejected)", event="skip", task=10)
log("TASK 12 skipped: requires ticket features (task 9 rejected)", event="skip", task=12)
